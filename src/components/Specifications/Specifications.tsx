...
export function Specifications() {
  ...
  const setPartRef = (id: string, key: keyof PartRefs) => (el: PartRefs[keyof PartRefs]) => {
    if (!partsRef.current[id]) {
      partsRef.current[id] = { dot: null, ring: null, path: null, label: null, number: null };
    }
  ...
  useGSAP(
    () => {
      ...
      const allLabels = parts.map((p) => p?.label).filter(Boolean) as HTMLDivElement[];
      const allPaths = parts.map((p) => p?.path).filter(Boolean) as SVGPathElement[];
      const allDots = parts.map((p) => p?.dot).filter(Boolean) as SVGCircleElement[];
      const allRings = parts.map((p) => p?.ring).filter(Boolean) as SVGCircleElement[];
      ...
      mm.add('(min-width: 768px) and (prefers-reduced-motion: no-preference)', () => {
        ...
        gsap.set(allRings, { scale: 0, opacity: 0, transformOrigin: 'center' });
        ...
        tl.fromTo(ghostRef.current, { xPercent: 4 }, { xPercent: -4, ease: 'none' }, 0);
        ...
        SPEC_CALLOUTS.forEach((c, i) => {
          ...
          tl.to(refs.path, { strokeDashoffset: 0, ease: 'none', duration: 0.12 }, seg);
          tl.to(refs.dot, { scale: 1, ease: 'back.out(2)', duration: 0.06 }, seg);
          tl.fromTo(
            refs.ring,
            { scale: 0, opacity: 0.8 },
            { scale: 2.4, opacity: 0, ease: 'power2.out', duration: 0.18 },
            seg
          );
          tl.to(refs.label, { opacity: 1, y: 0, ease: 'power2.out', duration: 0.1 }, seg + 0.05);
          ...
          tl.to(
            counter,
            {
              ...
              ease: 'none',
              onUpdate: () => {
                if (refs.number) refs.number.textContent = formatValue(counter.v);
              },
            },
            seg + 0.05
          );
      ...
      mm.add('(min-width: 768px) and (prefers-reduced-motion: reduce)', () => {
        gsap.set(headphoneRef.current, { scale: 1, yPercent: 0, rotate: 0 });
        ...
        if (railFillRef.current) gsap.set(railFillRef.current, { scaleX: 1 });
        settleFinalNumbers();
      });
    ...
    >
      ...
      >
        ...
        <div
          ref={ghostRef}
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-center select-none z-0"
        >
          <span className="font-schein font-bold uppercase leading-none tracking-[0.02em] text-[28vw] md:text-[20vw] text-white/[0.035] whitespace-nowrap">
            {SPEC_GHOST_WORD}
          </span>
        </div>
        ...
        <header className="relative z-20 px-6 pt-8 md:absolute md:top-10 md:left-12 md:p-0 max-w-xl">
          ...
          <p className="font-roboto text-sm md:text-base leading-relaxed text-white/60 mt-4 max-w-md text-pretty">
            {SPEC_INTRO}
          </p>
        </header>
        ...
        >
          {/* Headphone */}
          <div ref={headphoneRef} className="absolute inset-0 z-10 will-change-transform">
            <Image
            ...
            {SPEC_CALLOUTS.map((c) => (
              ...
              >
                ...
                >
                  ...
                  <span className="font-saira text-sm font-medium text-white/70">{c.unit}</span>
                </div>
        ...
        <div className="md:hidden relative z-20 grid grid-cols-2 gap-px bg-white/10 mt-8 mx-6 rounded-sm overflow-hidden">
          {SPEC_CALLOUTS.map((c) => (
            <div key={c.id} data-spec-card className="bg-[#101010] p-4">
              ...
              <div className="flex items-baseline gap-1 mt-2">
                ...
                <span className="font-saira text-xs font-medium text-white/70">{c.unit}</span>
              </div>
              <p className="font-roboto text-[11px] leading-snug text-white/45 mt-1">{c.detail}</p>
            </div>
          ))}
        </div>
          ...
          <ul className="flex flex-wrap items-center gap-x-6 gap-y-2 md:justify-between">
            {SPEC_SECONDARY.map((s) => (
              <li key={s.label} className="flex items-baseline gap-2">
                ...
                <span className="font-saira text-sm font-medium text-white/85">{s.value}</span>
              </li>
            ))}
          </ul>
        </div>
